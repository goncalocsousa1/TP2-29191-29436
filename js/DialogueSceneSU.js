export default class DialogueSceneSU extends Phaser.Scene {
    constructor() {
        super('DialogueSceneSU');
    ...
    init(data) {
        this.dialogueData = data.dialogueData || []; // Dialogue data passed from SecondUpScene
    }
    ...
    create() {
        if (!this.dialogueData.length) {
            ...
            this.scene.stop();
            this.scene.resume('SecondUpScene');
            return;
        }
    ...
    progressDialogue() {
        if (!this.isTyping) {
            if (this.currentLine < this.dialogueData.length - 1) {
                ...
                // End dialogue and resume SecondUpScene
                this.scene.stop();
                this.scene.resume('SecondUpScene');
            }
        }
